...
const ScrollAnimations = () => {
  ...
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden">
      ...
      {/* Wallet with Coins Animation */}
      <div 
        className="absolute bottom-1/3 left-1/6 w-40 h-40 opacity-30"
        style={{
      ...
      >
        <svg viewBox="0 0 120 120" className="w-full h-full">
          ...
          {Array.from({ length: 5 }).map((_, index) => {
            ...
            const coinX = 50 + (scrollY * 0.1 + delay * 30) % 60;
            const coinY = 30 - (scrollY * 0.05 + delay * 10) % 20;
            return (
              <circle
                key={index}
                cx={coinX}
                ...
                style={{
                  ...
                  opacity: Math.max(0, 1 - ((scrollY * 0.01 + delay) % 2) / 2),
                }}
              />
            );
          })}
          
          {/* More scattered coins */}
          {Array.from({ length: 3 }).map((_, index) => {
            const delay = index * 0.5 + 1;
            const coinX = 45 + (scrollY * 0.08 + delay * 25) % 40;
            const coinY = 25 - (scrollY * 0.03 + delay * 8) % 15;
            return (
              <ellipse
                key={`coin-${index}`}
                cx={coinX}
                ...
                style={{
                  animationDelay: `${delay}s`,
                  opacity: Math.max(0, 0.8 - ((scrollY * 0.008 + delay) % 3) / 3),
                }}
              />
            );
          })}
